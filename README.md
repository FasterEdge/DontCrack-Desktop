...
- 点击「添加实例」,再点击「刷新状态」即可查看该进程的管理器运行状态与日志
- 使用「启动进程 / 停止进程」按钮控制目标进程;「扫描全部」可一次性探测所有实例的在线情况
- 底部的「自动刷新」可开启/关闭轮询,调整 1s ~ 10s 的心跳刷新间隔
...
| 通信链路 | 前端 `invoke('dc_request')` → Rust `reqwest` → DontCrack HTTP 接口,支持超时控制与 password 鉴权 |

### 六、接口约定

DontCrack 各平台版本(Windows / Linux / Android / OpenHarmony)共享同一套 HTTP 接口,本工具直接复用:

| 接口        | 方法 | 说明                                              |
|-------------|------|---------------------------------------------------|
| `/startup`  | GET/POST | 启动进程,同时重置重启次数                    |
| `/heartbeat`| GET/POST | 获得心跳信息,输出启动情况和缓存中的日志      |
| `/shutdown` | GET/POST | 终止进程                                        |

开启密码时,请求在 URL 参数中携带 `password`,例如 `xxx/startup?password=123456`。
